...
φpdf = φ = get_samples_porosity(size)
Kpdf = K = get_samples_permeability(φpdf, size)
...
for index in range(size):
      parraywell, N = main(degree=2, btype="spline", elems=100, rw=0.1, rmax=1000, H=Hpdf[index], mu=0.31e-3, φ=φpdf[index], ctinv=1/ctpdf[index], k_int=Kpdf[index], Q=Qpdf[index], timestep=60, endtime=1800)
